...
def parse_listing(url):
    ...
    title = soup.find("h1").get_text(strip=True) if soup.find("h1") else "Nissan 350Z"
    price_elem = soup.select_one("[data-test='price']")
    price_eur = None
    if price_elem:
        try:
            ...
            price_eur = None
    year_elem = soup.select_one("[data-test='first-registration']")
    year = year_elem.get_text(strip=True) if year_elem else "?"
    loc_elem = soup.select_one("[data-test='seller-location']")
    loc = loc_elem.get_text(strip=True) if loc_elem else "?"
    return title, price_eur, year, loc

def run(seen_set):
    try:
        r = requests.get(f"https://www.autoscout24.com/lst/nissan/350-z?price_to={MAX_EUR}", timeout=10)
        r.raise_for_status()
        soup = BeautifulSoup(r.text, "lxml")
